  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      .then((data) => {
        ...
        const filteredOpinions = articleResults.filter(
          (item) => item.multimedia.length >= 4
        );

        const articlesAssortment = data[0].results.filter(
          (item) => item.multimedia.length >= 4
        );
        ...
        this.setState({
          ...
          bottomHomeNews: articlesAssortment.slice(
            half,
            articlesAssortment.length
          ),
